...
int utcpSend (connIdType connId, char *data, int dataLen) {
	...
	static unsigned long int seqNo_tx_data = 0;
	unsigned long int seqNo_ack = 0;
	...
	if(((strncmp(data,"ACK",3))) == 0) {
		acksqrxstr = &data[3];
		seqNo_ack = atoi(acksqrxstr);
		utcpPkt.header.seqNo = seqNo_ack;
		utcpPkt.header.flags = 1; //indicating Control
		PRINTDEBUG(printf("setting control flag in header\n"));
	} else if(((strcmp(data,"CONNECT"))) == 0) {
		utcpPkt.header.flags = 1; //indicating Control
		PRINTDEBUG(printf("setting control flag in header\n"));
	} else {
		PRINTDEBUG(printf("setting data flag in header\n"));
		utcpPkt.header.seqNo = seqNo_tx_data;
	...
	if(((strcmp(data,"ACK"))) == 0) {
		PRINTDEBUG(printf("Sent control data:%s\n",data));
		return 0; 
	} else if(((strcmp(data,"CONNECT"))) == 0) {
		PRINTDEBUG(printf("Sent control data:%s\n",data));
		return 0; 
...
int utcpRecv (connIdType connId, char *data, int dataLen) {
	...
	if(utcpPkt.header.flags == 0) {
		...
		sprintf(acksqstr,"%d",utcpPkt.header.seqNo);
		strcat(ackstr,acksqstr);
		if (utcpSend(connId,ackstr, strlen(ackstr)) < 0) {
			printf("ERROR: %d.\n", errno);
			utcpClose(connId);
			return 0;
		}
		...
		PRINTDEBUG(printf("Received control\n"));
		memcpy(data,"ACK",strlen("ACK"));
		printf("recv: %s\n",data);
		return 0;
	}
